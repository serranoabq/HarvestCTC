<?php
	...
	class harvest_Contact_Widget extends WP_Widget {
		...
		function harvest_Contact_Widget() {
			$widget_ops = array(
				...
				'description' => __( 'Contact Info', 'harvest-ctc' ) 
			);
			$this->WP_Widget( 'widget_contact_info', __( 'Contact Info', 'harvest-ctc' ), $widget_ops);
		}
		function widget( $args, $instance ) {
			extract( $args );
			$title = apply_filters('widget_title', empty( $instance['title'] ) ? __( 'Contact Info', 'harvest-ctc' ) : $instance['title'], $instance, $this->id_base);
	...
	<?php } 
		...
		?>
			<p>
				<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Widget Title:', 'harvest-ctc' ); ?></label> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
			</p>
	<?php 
		} 
	} 
